...
type SH = 
    Provided.TypeProviderFile<"../../../Examples/SH2//FSM/P.txt" // Fully specified path to the scribble file
                               ,"SH" // name of the protocol
                               ,"P" // local role
                               ,"../../../Examples/SH2/configSP.yaml" // config file containing IP and port for each role and the path to the scribble script
                               ,Delimiter=delims 
...
let rec calcClipPoints (vert: int list)  (c:SH.State36) =
    ...
    | hd1::hd2::tail -> 
        let c1 = c.sendAbove(R, 1).receiveRes(R, res1).sendAbove(R, 2)
                  .receiveRes(R, res2)
        let s = res1.getValue() + res2.getValue()
        //let f = if s > 0 then true else false 
        let c2 = c1.sendInersect(R).receiveRes(R, res3)
        calcClipPoints (hd2::tail) c2
        
let polygon = {1..1000} |> Seq.toList

let dum = new DomainModel.Buf<int>()    
let startC = sh.Start().sendhello(R, 1).receivehello(R, dum).sendplane(R, 1)
TimeMeasure.start()
