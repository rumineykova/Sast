...
let getChoiceTransitions index (fsmInstance:ScribbleProtocole.Root []) = 
  ...
  let transition = 
      match methodName with 
        | "choice_send" ->  CFSMModel.Transition.Send (role, label)
        | "choice_receive" ->  CFSMModel.Transition.Recv (role, label)
  ...
  | h::t -> 
      ...
      if index = -1 then 
          ...
          | "send" | "receive" | "finish" ->
              let transition = 
                  match methodName with 
                  | "send" ->  CFSMModel.Transition.Send (role, label)
                  | "receive" ->  CFSMModel.Transition.Recv (role, label)
                  | "finish" ->  CFSMModel.Transition.End 
